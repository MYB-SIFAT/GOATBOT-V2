...
module.exports = {
        config: {
                ...
                category: "owner",
                guide: { en: "{pn} load | loadall | unload | reload | reloadall | list | info | install | autopush on/off" }
        },
        ...
        onStart: async function ({ args, message, event, commandName, getLang }) {
                ...
                if (sub === "list") {
                        ...
                        for (const e of evts) {
                                ...
                                categories[cat].push(e.config.name);
                        }
                        ...
                        lines.push(`│ ᴛᴏᴛᴀʟ: ${evts.length} ᴇᴠᴛ(ꜱ)`);
                ...
                if (sub === "info") {
                        ...
                        return message.reply(getLang("info",
                                ...
                                hooks.length ? hooks.join(", ") : "ᴏɴꜱᴛᴀʀᴛ", (cfg.aliases?.length ? cfg.aliases.join(", ") : "─")
                        ));
                }
        ...
        onReaction: async function ({ Reaction, message, event, getLang }) {
                ...
                const { nameNoExt, rawCode, destPath } = Reaction.data;
                const r = loadOne(nameNoExt, rawCode);
                if (r.status !== "success") return message.reply(getLang("installFail", r.name || nameNoExt, r.error.message));
                const rel = destPath.replace(process.cwd(), "");
                try {
