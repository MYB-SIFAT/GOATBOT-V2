...
const modified = ("Modified by SIFAT");
...
async function switchToNextAccount(reason = "Account issue detected") {
        ...
        multiAccountManager.penalizeAccount(multiAccountManager.getCurrentAccount(), reason);

        if (!multiAccountManager.hasMoreAccounts()) {
                if (multiAccountManager.allAccountsDead()) {
                        ...
                        if (!revived) {
                                log.warn("MULTI_ACCOUNT", "Emergency: no accounts can be revived. Waiting...");
                        ...
                        log.warn("MULTI_ACCOUNT", `Emergency revived: ${path.basename(revived)}`);
                } else {
        ...
        const fromAccount = path.basename(multiAccountManager.getCurrentAccount() || "unknown");
        log.warn("MULTI_ACCOUNT", `${reason} — switching to best available account...`);
        ...
        try {
                ...
                const best = multiAccountManager.selectBestAccount(true);
                if (best) {
                        const cookieCheck = await multiAccountManager.validateCookie(best);
                        if (!cookieCheck.valid) {
                                log.warn("MULTI_ACCOUNT", `Cookie validation failed for ${path.basename(best)}: ${cookieCheck.reason} — skipping`);
                                multiAccountManager.penalizeAccount(best, `Invalid cookie: ${cookieCheck.reason}`, "loginFail");
                        }
                }

                const nextAccount = multiAccountManager.switchToBest() || multiAccountManager.nextAccount();
                global.client.dirAccount = nextAccount;

                const toAccount = path.basename(nextAccount);
                log.info("MULTI_ACCOUNT", `Switched to: ${toAccount} (health=${multiAccountManager.health[nextAccount]})`);
